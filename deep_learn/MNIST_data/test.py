...
def model_sigmoid(x,weight,biase,drop = 1):
    ...
    prediction = tf.nn.dropout(prediction,drop)
    return prediction,output
def model_relu(x,weight,biase,drop = 1):
    output =     tf.matmul(x,weight)+biase
    prediction = tf.nn.relu(output)
    prediction = tf.nn.dropout(prediction,drop)
    return prediction,output

prediction_1,output_1 = model_relu(x,weights_1,biases_1,p_keep_input)
prediction_2,output_2 = model_relu(prediction_1,weights_2,biases_2,p_keep_hidden)
